...
/* ------------------------------------------------------------------------ */
static unsigned char *in_buffer, *in_end, uiclip[1024], *uiclp = NULL;

/* M2Suite: the original raw-pointer macros read past the end of the
 * input buffer on truncated/corrupt frames — a hard access violation
 * that no C++ try/catch can intercept (the "crash that never reaches
 * the log"). Replaced with bounds-checked equivalents that return zero
 * bytes at EOF; the per-chunk size countdowns still terminate. */
static unsigned char get_byte(void) {
	return (in_buffer < in_end) ? *in_buffer++ : 0;
}
static void skip_byte(void) {
	if (in_buffer < in_end) in_buffer++;
}
static unsigned short get_word(void) {
	unsigned short hi = get_byte();
	unsigned short lo = get_byte();
	return (unsigned short)((hi << 8) | lo);
}
static unsigned long get_long(void) {
	unsigned long hi = get_word();
	unsigned long lo = get_word();
	return (hi << 16) | lo;
}
...
void decode_cinepak_free(void* context)
{
  ...
  /* M2Suite: do NOT null uiclp here. It points into the static `uiclip`
   * array and is SHARED across all decoder instances. Freeing one decoder
   * used to null the clip table that a freshly-constructed decoder still
   * needs, so replaying any Cinepak video crashed with a NULL deref inside
   * the pixel writers. The table is process-lifetime state — leave it. */
  if(!cvinfo) return;
  for(i = 0; i < MAX_STRIPS; i++)
			{
...
void (*cvid_v4)(unsigned char *frm, unsigned char *end, int stride, cvid_codebook *cb0,
	...
	y_bottom = 0;
	in_buffer = buf;
	in_end = buf + size; /* M2Suite: bound for the checked readers above */
	...
	for(cur_strip = 0; cur_strip < strips; cur_strip++)
		...
		/* M2Suite: restored the keyframe-flag condition (it was commented
		 * out in this copy). Per the cvid spec and FFmpeg's cinepak.c,
		 * strips inherit the previous strip's codebooks only when frame
		 * flag bit 0 is CLEAR; when set, each strip keeps its own
		 * codebooks from the previous frame. Unconditional copying
		 * clobbered strip 1 on every inter frame — corrupting the bottom
		 * half of every 2-strip movie (all of strahl). */
		...
		/* M2Suite: removed a "return 8" rejection of frames whose first
		 * strip rect doesn't equal the full frame — strip rects are
		 * per-strip bands (y1 = strip height), so that check broke every
		 * multi-strip movie (all of strahl's 2-strip 288x216 films).
		 * The original Ferguson code accumulates strips via y_bottom. */
		...
		while(top_size > 0)
			{
                        ...
                        if(in_buffer >= in_end) break; /* M2Suite: clamp chunk jumps */
			chunk_id  = get_word();
			chunk_size = get_word();
			...
			switch(chunk_id)
				...
				case 0x3000: 
					while((chunk_size > 0) && (y < y_bottom))
						{
                                                /* M2Suite: removed guard that skipped the final block row (bottom-right corner artefact) */
						flag = get_long();
						chunk_size -= 4;
				...
				case 0x3100:
					/* M2Suite: rewritten to the canonical Cinepak inter-frame
					 * vector decode (matches FFmpeg cinepak.c and ScummVM).
					 * The previous variant tracked the flag mask with a
					 * non-standard `if(mask==1)` refill that lost sync at
					 * 32-bit flag-word boundaries, occasionally drawing a
					 * wrong codebook entry into a block — most visible as the
					 * static garbage block in the bottom-right corner of many
					 * videos. Here mask starts at 0 so the first `mask >>= 1`
					 * forces a fresh flag load; the skip bit and (when coded)
					 * the V1/V4 selector bit are each pulled with a refill at
					 * the point of use, exactly as the reference decoders do. */
					flag = 0;
					mask = 0;
					while(y < y_bottom)
						{
						/* skip-flag bit for this block */
						if(!(mask >>= 1))
							{
							if(chunk_size < 4) break;
						...
						if(flag & mask)		/* coded (non-SKIP) */
							...
							}		/* else SKIP: keep previous frame's pixels */
